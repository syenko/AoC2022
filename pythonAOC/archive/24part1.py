...
while len(checkNext) > 0:
    ...
    added = False
    # right
    if x + 1 < WIDTH and (x + 1, y) not in pos and y >= 0:
        if ((x + 1, y), turn) not in checking:
            checkNext.append(((x + 1, y), turn))
            checking.add(((x + 1, y), turn))
        added = True
    # down
    if (y + 1 < HEIGHT or x == WIDTH - 1) and (x, y + 1) not in pos:
    ...
    # left
    if x - 1 >= 0 and (x - 1, y) not in pos and y >= 0:
        if ((x - 1, y), turn) not in checking:
            checkNext.append(((x - 1, y), turn))
            checking.add(((x - 1, y), turn))
    # up
